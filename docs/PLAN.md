...
- 화면별 적용:
  ...
  - **인용구 캡처**: 사진을 찍은 뒤 인용할 영역을 드래그로 표시하고 "텍스트 인식"을 누르면, 크롭된 영역에 대해 Latin/Korean 두 인식기가 동시에 처리되는 동안 `LoadingOverlay`("텍스트 인식 중…")로 화면을 덮음(중복 인식 방지) → 완료 시 인식 결과가 편집 가능한 텍스트필드에 채워짐. "다음 페이지 이어서 촬영"으로 추가 촬영할 때도 동일하게 적용.
...
4. **책 상세**: 메타데이터, 진행률, 라운드별 진행 이력/독후감, 인용구 목록. 딥다이브용 화면이며 빠른 기록 시트에서 "상세보기"로 진입하거나, 검색/목록에서 직접 진입. **진행 이력 목록의 각 로그 행은 탭하면 인라인으로 펼쳐져 수정/삭제 가능**(대시보드 빠른 기록 시트와 동일한 `EditLogUseCase`/`DeleteLogUseCase` 재사용) — 과거 특정 날짜의 기록을 고치는 것은 이 화면이 담당. 이력의 각 행은 책 `format`에 따라 "245p" 또는 "62%"로 표시(저장된 값은 항상 페이지, 표시만 환산).
   ...
   - → 독후감작성 등 저빈도 작업으로 이동
5. **인용구 캡처**: 대시보드 빠른 기록 시트 또는 책 상세에서 진입(둘 다 책 컨텍스트를 이미 알고 있어 책 선택 단계 없음). 단발 촬영(연속 프레임 아님, 정확도 우선) → 촬영된 사진 위에서 **인용할 영역을 손가락 드래그로 사각형 표시** → 그 잘라낸 영역에만 Latin+Korean 인식기를 동시 실행해 더 완전하게 인식된 쪽(글자 수가 더 많은 쪽, 동률이면 한국어 우선)을 채택 → 인식 결과가 편집 가능한 텍스트필드에 채워짐. (초기 설계였던 "인식된 줄을 체크박스 리스트로 선택" 방식은 실기기에서 OCR 줄바꿈이 지저분하게 나와 오히려 헷갈린다는 이유로, 사용자가 직접 인용 영역을 지정하는 드래그 크롭 방식으로 변경됨.)
   - **여러 페이지에 걸친 인용구**: 텍스트필드 아래 "다음 페이지 이어서 촬영" 버튼을 둔다(최소 한 번 인식에 성공해야 활성화). 탭하면 카메라가 다시 열려 다음 페이지를 촬영 → 같은 드래그 크롭+OCR 과정을 반복 → 그 결과가 "n페이지" 항목으로 추가되고, 지금까지의 모든 페이지 텍스트를 촬영 순서대로 이어붙인 값이 하나의 편집 가능한 텍스트필드에 표시됨(순수 함수 `joinQuotePages`로 계산, 안드로이드 의존성 없어 단위 테스트 가능). 페이지 수 제한 없이 반복 가능. 인용구는 항상 1건으로 저장됨(페이지별로 쪼개지지 않음).
...
- **OCR**: 실시간 오버레이 대신 정지 이미지 캡처 후 처리(흔들림 있는 실시간 인식보다 안정적). 인식된 줄을 체크박스 리스트로 선택하는 방식을 초기에 고려했으나, 실기기에서 줄바꿈 인식이 지저분하게 나와 오히려 사용성이 떨어져 **사용자가 사진 위에서 인용 영역을 직접 드래그로 잘라내면 그 영역만 OCR** 하는 방식으로 변경(`HighlightQuoteContent`의 크롭 사각형 → `QuoteOcrProcessor.recognize(bitmap, cropRect)`). 인식 결과는 항상 편집 가능한 텍스트필드에 표시되어 오탈자를 직접 고칠 수 있음. 드래그 크롭 후 "텍스트 인식" 버튼을 눌렀을 때 Latin+Korean 두 인식기를 동시 실행하고 더 길게 인식된 쪽을 채택(`pickBetterQuoteText`, 동률이면 한국어 우선).
...
- **여러 페이지 인용구("다음 페이지 이어서 촬영")**: `QuoteCaptureViewModel`이 `capturedPages: List<CapturedQuotePage>`(각 원소 = 한 번의 드래그 크롭+OCR 결과 텍스트 + 그 페이지의 인식/입력된 페이지번호)를 촬영 순서대로 보관. "다음 페이지 이어서 촬영" 탭 시 카메라를 다시 열고 새 크롭+OCR 결과를 리스트에 append하며, 최종 텍스트필드는 순수 함수 `joinQuotePages(capturedPages): String`(각 페이지 텍스트를 촬영 순서대로 빈 줄로 join, Android 의존성 없어 단위테스트 가능)의 결과를 표시하되 사용자가 직접 수정 가능. 저장 시 `Quote.pageNumber = capturedPages.first().pageText`, `pageNumberEnd = capturedPages.last().pageText`(캡처가 2개 이상이고 첫 페이지 번호와 다를 때만, 아니면 null). "계속 촬영"(저장 후 새 인용구)과는 별개 기능이며 저장 전에만 등장.
...
- **미구현(다음 작업)**: 통계 화면(장르/작가/출판사/국가별), 런처 아이콘. CI가 `:app` 유닛테스트(`BookMetadataRepositoryImplTest`, `ReminderSchedulerTest`, `QuoteCaptureViewModelTest`, `QuoteOcrProcessorTest` 등)를 아직 실행하지 않음(`:domain:test`만 돎) — 워크플로에 스텝 추가 필요.
