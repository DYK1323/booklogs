...
- **ReadingRoundEntity** (`reading_rounds`, **스키마 버전 3에서 `startingPage` 추가**): id, bookId(FK), roundNumber, startedAt, finishedAt?, **endReason?**(COMPLETED/DROPPED, 라운드가 끝났을 때만 값이 들어가고 진행/일시중지 중엔 null), **startingPage**(Int, 기본 0) — 재독마다 새 라운드 생성. `startingPage`는 그 라운드의 첫 로그가 기준으로 삼는 델타 baseline이며, 책 상세의 "라운드 이력" 화면에서 언제든 직접 고칠 수 있는 유일한 편집 지점이다 — 예를 들어 완독/중단을 실수로 눌러 라운드가 의도치 않게 새로 갈렸을 때도(원래 읽던 페이지가 165였다면) 새 라운드의 `startingPage`를 165로 고쳐주면 그 시점부터의 델타가 다시 올바르게 계산된다(별도의 "라운드 병합" 로직 없이 뺄셈 기준값 하나만 고치면 됨). 재독 시작 시 새 라운드를 만들면(기본 `startingPage = 0`) 페이지 델타 계산도 라운드 내에서만 비교하므로 "재독으로 페이지가 1로 돌아갈 때 음수 델타" 같은 엣지케이스가 자연히 해결되고, 중간까지 읽다가 처음부터 다시 읽고 싶은 경우는 `startingPage`를 0으로 둔 채(기본값) 새 라운드를 시작하면 된다.
  ...
  - **라운드 이력 편집 화면**(책 상세): 모든 라운드를 목록으로 보여주고 각 라운드를 인라인으로 펼쳐 시작일/종료일/종료사유/시작 페이지를 수정할 수 있다. 단, 라운드의 열림/닫힘 자체(진행 중 ↔ 종료됨)는 이 화면에서 토글할 수 없고 항상 책 상태 전이(완독/중단/다시 읽기)를 통해서만 바뀐다 — 날짜·사유·시작 페이지 같은 "기록 정정"과 "라운드가 진행 중인지"라는 상태 머신을 명확히 분리하기 위함.
