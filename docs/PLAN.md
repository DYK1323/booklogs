...
7. 독후감은 책 단위로 관리 (원래는 "재독 시마다 새 독후감"이 목표였으나, 실사용 중 라운드 경계가 자주 흔들리는 문제가 드러나 최종적으로는 라운드에 종속되지 않는 책 단위 독후감으로 정착 — 아래 ReviewEntity, 화면 흐름 #6 참고)
...
- **ReadingRoundEntity** (`reading_rounds`, **스키마 버전 3에서 `startingPage` 추가**): id, bookId(FK), roundNumber, startedAt, finishedAt?, **endReason?**(COMPLETED/DROPPED, 라운드가 끝났을 때만 값이 들어가고 진행/일시중지 중엔 null), **startingPage**(Int, 기본 0) — 재독마다 새 라운드 생성. `startingPage`는 그 라운드의 첫 로그가 기준으로 삼는 델타 baseline이며, 책 상세의 "라운드 이력" 화면에서 언제든 직접 고칠 수 있는 유일한 편집 지점이다 — 예를 들어 완독/중단을 실수로 눌러 라운드가 의도치 않게 새로 갈렸을 때도(원래 읽던 페이지가 165였다면) 새 라운드의 `startingPage`를 165로 고쳐주면 그 시점부터의 델타가 다시 올바르게 계산된다(별도의 "라운드 병합" 로직 없이 뺄셈 기준값 하나만 고치면 됨). 재독 시작 시 새 라운드를 만들면(기본 `startingPage = 0`) 페이지 델타 계산도 라운드 내에서만 비교하므로 "재독으로 페이지가 1로 돌아갈 때 음수 델타" 같은 엣지케이스가 자연히 해결되고, 중간까지 읽다가 처음부터 다시 읽고 싶은 경우는 `startingPage`를 0으로 둔 채(기본값) 새 라운드를 시작하면 된다.
  - **완독과 중단은 같은 메커니즘**(라운드 종료, `finishedAt` 기록)이고 `endReason`만 다름. 일시중지(PAUSED)는 라운드를 종료하지 않고 그대로 열어둔 채 책 상태만 바꾸는 것이라 서로 구분됨.
  - FINISHED/DROPPED 상태에서 다시 읽기 시작하면(재독/재도전) 기존 라운드는 그대로 두고 `roundNumber`가 1 증가한 **새 라운드**를 생성 — 1차 시도의 진행 이력은 보존되고 2차 시도가 새로 쌓임(독후감은 더 이상 라운드에 종속되지 않음, 아래 ReviewEntity 참고). PAUSED에서 재개하면 라운드를 새로 만들지 않고 열려있던 라운드를 그대로 이어감.
  - **라운드 이력 편집 화면**(책 상세): 모든 라운드를 목록으로 보여주고 각 라운드를 인라인으로 펼쳐 시작일/종료일/종료사유/시작 페이지를 수정할 수 있다. 단, 라운드의 열림/닫힘 자체(진행 중 ↔ 종료됨)는 이 화면에서 토글할 수 없고 항상 책 상태 전이(완독/중단/다시 읽기)를 통해서만 바뀐다 — 날짜·사유·시작 페이지 같은 "기록 정정"과 "라운드가 진행 중인지"라는 상태 머신을 명확히 분리하기 위함. **삭제도 같은 원칙**: 종료된(닫힌) 라운드만 삭제할 수 있고(그 라운드의 진행 이력도 함께 삭제됨, `onDelete=CASCADE`), 현재 진행 중인 라운드는 삭제할 수 없다 — 그걸 지우면 READING/PAUSED 상태인 책에 대응하는 열린 라운드가 없어져 상태와 데이터가 어긋나기 때문.
- **ReadingLogEntity** (`reading_logs`): id, bookId(FK), readingRoundId(FK), currentPage, logDateEpochDay(타임존 안정적인 일자 버켓용), loggedAt — bookId, logDateEpochDay에 인덱스. **델타는 컬럼으로 저장하지 않는다.** 각 로그는 "이 시점에 몇 페이지였다"는 스냅샷일 뿐이고, 페이지 증가량(델타)은 항상 같은 라운드의 로그들을 `loggedAt` 순으로 정렬해 인접한 두 로그의 차이로 그때그때 계산한다(첫 로그는 그 라운드의 `startingPage`를 기준으로, clamped ≥0). 이렇게 델타를 파생값으로만 다루면 과거 로그를 수정/삭제해도 그 행 하나만 갱신/삭제하면 끝이고, 이웃 로그의 저장된 델타를 다시 써넣는 연쇄 작업이 필요 없다 — **어떤 시점의 로그든 자유롭게 수정/삭제 가능**. 개인 독서기록 규모(수백~수천 행)에서 매번 정렬+차이계산하는 비용은 무시할 수준.
...
- **ReviewEntity** (`reviews`, **스키마 버전 3에서 `readingRoundId` 제거**): id, bookId(FK), content, rating?, createdAt — 독후감은 라운드가 아니라 **책 단위**로 존재한다. 델타 계산과 달리 독후감 자체는 "재독마다 리셋되는 값"이 아니고, 라운드 경계(완독/중단/다시 읽기)가 실수로 바뀌어도 독후감이 엉뚱한 라운드에 묶여 사라지거나 잘못 그룹핑될 이유가 없어야 하기 때문. 작성 화면(`ReviewEditorViewModel`)은 진입 시 그 책의 가장 최근 독후감이 있으면 불러와 수정 모드로 시작한다.
...
com.dyk1323.booklogs/
  ...
  domain/
    ...
    usecase/
      ComputeLogDeltasUseCase.kt      ← 핵심 파생 로직, 순수 함수: 한 라운드의 로그 목록(loggedAt순)과 그 라운드의 `startingPage`(기본 0)를 받아, 첫 로그는 `startingPage` 대비로, 이후는 인접 쌍의 차이로 delta를 계산(음수 클램프). AggregateDailyPagesUseCase와 라운드 진행 이력 표시(책 상세) 양쪽에서 재사용. `startingPage`를 고치는 것만으로 델타를 다시 정정할 수 있어, 완독/중단 실수로 라운드가 잘못 갈렸을 때의 유일한 정정 지점이기도 하다.
      AggregateDailyPagesUseCase.kt   ← 핵심 요구사항(#1), 순수 함수, 최우선 단위테스트 대상. 전체 로그를 라운드별로 묶어 각 라운드의 `startingPage`를 baseline으로 ComputeLogDeltasUseCase로 델타를 얻은 뒤 각 델타를 "뒤쪽 로그"의 logDateEpochDay에 귀속시켜 날짜별 합산(요청 기간 밖의 이전 로그도 델타 계산엔 포함해야 경계의 첫 델타가 정확함)
      ...
      EditRoundUseCase.kt             ← 책 상세 "라운드 이력" 화면에서 라운드의 startedAt/finishedAt/endReason/startingPage를 정정. 라운드의 열림/닫힘 상태 자체는 바꿀 수 없음(finishedAt/endReason은 이미 종료된 라운드에서만, 그리고 둘이 함께만 변경 가능) — 그 전이는 항상 ChangeBookStatusUseCase를 통해서만 일어남
      DeleteRoundUseCase.kt           ← 같은 화면에서 라운드 자체를 삭제(그 라운드의 로그는 onDelete=CASCADE로 함께 삭제). 종료된 라운드만 삭제 가능 — 열린 라운드를 삭제하면 READING/PAUSED 책에 대응 라운드가 없어져 상태 머신과 어긋나므로 거부
  ...
  ui/
    ...
    detail/ (BookDetailScreen, BookDetailViewModel, 라운드 이력 편집(RoundRow — 라운드별 인라인 펼침/편집, EditRoundUseCase 호출), QuoteListScreen/ReviewListScreen(각각 "전체보기" 진입, BookDetailViewModel을 공유해 검색/필터만 화면 로컬 상태로 추가), BookStatusActions, DeleteBookConfirmDialog — "진행률 기록"/"인용구 추가" 버튼은 dashboard의 BookQuickActionSheet/QuoteCaptureScreen을 그대로 재사용)
...
4. **책 상세**: 메타데이터, 진행률, **라운드 이력**(전체 라운드 목록 + 인라인 편집), 진행 이력, 인용구/독후감. 딥다이브용 화면이며 빠른 기록 시트에서 "상세보기"로 진입하거나, 검색/목록에서 직접 진입. **진행 이력 목록의 각 로그 행은 탭하면 인라인으로 펼쳐져 수정/삭제 가능**(대시보드 빠른 기록 시트와 동일한 `EditLogUseCase`/`DeleteLogUseCase` 재사용) — 과거 특정 날짜의 기록을 고치는 것은 이 화면이 담당. 이력의 각 행은 책 `format`에 따라 "245p" 또는 "62%"로 표시(저장된 값은 항상 페이지, 표시만 환산).
   - **인용구/독후감 미리보기 + 전체보기**: 둘 다 내용이 길게 쌓이는 콘텐츠라, 책 상세에는 최신순 **3개까지만** 카드로 미리 보여주고(하나라도 더 있으면) 섹션 제목 옆에 "전체보기" 버튼을 노출한다. 탭하면 각각 `QuoteListScreen`/`ReviewListScreen`(둘 다 `ui/detail`)으로 이동 — 별도 데이터 로딩 없이 책 상세와 같은 `BookDetailViewModel` 인스턴스를 그대로 공유해 구독 중인 목록을 재사용한다. `QuoteListScreen`은 내용/페이지 번호로 검색, `ReviewListScreen`은 내용 검색 + 평점(전체/★1~★5) 필터를 제공. 편집/삭제/댓글(인용구)은 책 상세와 동일한 뷰모델 메서드를 그대로 호출하므로 어느 화면에서 조작하든 상태가 일관됨.
   - **라운드 이력**(신설): "상태" 섹션 바로 아래 배치. 각 라운드를 "N번째 라운드 · 2026.07.11 ~ 진행 중 · 165p부터"처럼 한 줄로 보여주고, 탭하면 인라인으로 펼쳐져 시작일/종료일/종료사유(닫힌 라운드만)/시작 페이지를 텍스트필드로 직접 고칠 수 있다(`EditRoundUseCase`). 라운드의 진행 중/종료 여부 자체는 여기서 바꿀 수 없고 항상 아래 "상태 변경 액션"을 통해서만 바뀐다. 완독/중단을 실수로 눌러 라운드가 의도치 않게 갈렸을 때, 예전에는 "라운드 병합" 전용 usecase(`UndoRoundSplitUseCase`)로 되돌렸지만 지금은 이 화면에서 새 라운드의 시작 페이지를 원래 읽던 페이지로 고쳐주는 것만으로 충분하므로 그 usecase는 제거됨(아래 변경 이력 참고). 펼친 라운드가 **닫힌 라운드**면 "취소"/"저장" 버튼 옆에 "삭제" 버튼도 나타나(`DeleteRoundUseCase`), 확인 다이얼로그("이 라운드에 기록된 진행 이력도 함께 삭제돼요")를 거쳐 그 라운드와 딸린 로그를 통째로 지울 수 있다 — 현재 진행 중인 라운드는 이 버튼 자체가 나타나지 않는다.
   - **상태 변경 액션**(`BookStatusActions`, `ChangeBookStatusUseCase` 호출): READING 중엔 "다 읽음" / "중단" / "일시중지" 세 버튼을 노출. "일시중지"는 라운드를 유지한 채 상태만 바꾸고, "다 읽음"/"중단"은 현재 라운드를 종료(각각 `endReason = COMPLETED`/`DROPPED`)하고 책 상태를 FINISHED/DROPPED로 바꿈. FINISHED/DROPPED/PAUSED 상태에선 "다시 읽기 시작" 버튼 하나로 재개(PAUSED는 같은 라운드 이어감, FINISHED/DROPPED는 새 라운드 시작, 새 라운드의 `startingPage`는 기본 0 — 처음부터 다시 읽는 게 기본 가정이며, 중간부터 이어 읽고 싶다면 위 "라운드 이력"에서 시작 페이지를 직접 고치면 됨). **PLANNED 상태에선 "읽기 시작" 버튼**(라운드가 아직 하나도 없는 책이라 첫 라운드를 생성 — FINISHED/DROPPED→READING과 동일한 메커니즘, 라벨만 다름). 등록/상태변경처럼 저빈도 작업이라 책 상세에 위치, "다 읽음"/"중단"은 라운드가 종료된다는 사실을 알리는 확인 다이얼로그를 거친 뒤 적용(그 외 전이는 확인 다이얼로그 없이 즉시 적용).
...
6. **독후감 작성**: 책 단위로 텍스트+평점(선택) 작성/저장(`ReviewEditorViewModel`) — 라운드에 종속되지 않으므로 진행 중이든 완독/중단/일시중지 상태든 언제나 남길 수 있음. 독후감은 책 하나에 **여러 개 쌓일 수 있다**(재독마다, 혹은 같은 라운드 안에서도 생각이 바뀌면 새로 남길 수 있음) — "작성" 버튼(책 상세/독후감 전체보기 화면 공통)은 항상 새 독후감 작성 모드로 진입하고, 기존 독후감을 고치려면 그 카드의 "수정" 버튼을 눌러 특정 `reviewId`를 지정해 편집 모드로 진입해야 한다(`ReviewEditorViewModel.start(bookId, reviewId)` — `reviewId`가 없으면 항상 새 글, 있으면 그 글만 불러옴 — 화면이 "가장 최근 것"을 알아서 추측하지 않음).
...
- **라운드/독후감 아키텍처 재설계 — 라운드 이력 편집 화면 신설, `startingPage` 도입, 독후감 책 단위로 전환**: 위 "라운드 분할 되돌리기(가벼운 복구)" 항목의 `UndoRoundSplitUseCase`/실행취소 스낵바를 붙인 지 얼마 안 돼, "예전 라운드 이력도 언제든 조회/편집할 수 있으면 좋겠다"는 요청으로 처음부터 다시 설계함 — 스낵바 시점에만 되돌릴 수 있는 임시방편 대신, 책 상세에 라운드 이력을 상시 조회/편집하는 화면을 두는 쪽이 근본적으로 더 낫다고 판단. 결과적으로 **`UndoRoundSplitUseCase`와 그 스낵바/이벤트 채널은 완전히 제거**되고 다음으로 대체됨:
  - `ReadingRoundEntity`에 `startingPage`(기본 0) 추가(스키마 버전 3) — 그 라운드의 첫 로그가 델타를 계산할 때 쓰는 baseline. "라운드 이력" 화면에서 언제든 직접 고칠 수 있으므로, 완독/중단을 실수로 눌러 라운드가 잘못 갈렸을 때도 라운드를 병합할 필요 없이 새 라운드의 `startingPage`를 원래 읽던 페이지로 고쳐주는 것만으로 델타가 다시 맞게 계산됨. `ComputeLogDeltasUseCase`/`AggregateDailyPagesUseCase`가 이 baseline을 반영하도록 시그니처 변경, `EditRoundUseCase`도 `startingPage` 파라미터를 받도록 확장(새 검증: `startingPage >= 0`).
  - `ReviewEntity`에서 `readingRoundId` FK를 완전히 제거(스키마 버전 3) — 독후감은 더 이상 재독마다 리셋되는 라운드에 종속되지 않고 책 단위로 존재. `ReviewEditorViewModel.start()`는 이제 그 책의 가장 최근 독후감을 찾아(있으면) 수정 모드로 불러옴("현재 라운드에 독후감이 있는지" 확인하던 이전 로직 대체).
  - 책 상세에 새 "라운드 이력" 섹션 신설: 전체 라운드를 목록으로 보여주고 각 라운드를 인라인으로 펼쳐 시작일/종료일/종료사유/시작 페이지를 수정 가능(`EditRoundUseCase`) — 단, 라운드의 진행 중/종료 여부 자체는 이 화면에서 토글할 수 없고 항상 "상태 변경 액션"을 통해서만 바뀜(기록 정정과 상태 머신을 분리).
  - Room 마이그레이션: `MIGRATION_2_3`이 `reading_rounds`에 `starting_page` 컬럼을 추가(`ALTER TABLE ... ADD COLUMN`)하고, `reviews`는 SQLite가 컬럼 삭제를 직접 지원하지 않으므로 새 테이블 생성 → 데이터 복사 → 기존 테이블 삭제 → 이름 변경 패턴으로 `reading_round_id` 없이 재구성.
  - 백업(`BackupEnvelope`)도 함께 갱신: `ReadingRoundBackupDto`는 `startingPage: Int = 0` 기본값을 둬서 이전 백업 파일(그 키가 없음)도 그대로 가져오기 가능, `ReviewBackupDto`는 `readingRoundId`를 제거하되 `kotlinx.serialization`의 `ignoreUnknownKeys = true` 덕분에 이전 백업 파일에 남아있는 그 키는 그냥 무시됨 — 둘 다 `schemaVersion`을 올리지 않고도 하위 호환됨.
- **인용구/독후감 전체보기 화면 신설 + 독후감 다건화**: 인용구/독후감 둘 다 내용이 길어질 수 있는 콘텐츠인데 책 상세 화면에 전부 나열되면 다른 섹션(라운드 이력/진행 이력)까지 스크롤이 길어지는 문제가 있어, 책 상세에는 최신순 **3개까지만** 미리보기하고 "전체보기"를 누르면 검색/필터가 되는 별도 화면으로 이동하도록 재구성. `QuoteListScreen`/`ReviewListScreen`(둘 다 신규, `ui/detail`)은 책 상세와 같은 `BookDetailViewModel` 인스턴스를 그대로 재사용하고(별도 데이터 로딩 없음), 검색어/평점 필터만 화면 로컬 `remember` 상태로 둠. 기존에 `BookDetailScreen.kt` 파일 안에 `private`로 있던 `QuoteCard`/`ReviewCard`/`QuoteCommentsSheetContent`/`quotePageLabel`/`reviewTitle`/`formatDate`는 두 화면에서 재사용해야 해서 `internal`로 열었고, 인용구 인라인 수정 폼도 `QuoteEditForm`으로 추출해 공유.
  - 위 "라운드/독후감 아키텍처 재설계"에서 독후감을 책 단위로 옮기며 `ReviewEditorViewModel.start()`가 "그 책의 가장 최근 독후감을 자동으로 불러와 수정"하는 동작으로 바뀌었는데, 그 상태로는 독후감이 사실상 책마다 최신 1개로 고정돼(두 번째 독후감을 만들 방법이 없음) "전체보기" 목록의 의미가 없었음 — 그래서 이번에 `start(bookId, reviewId: Long?)`로 시그니처를 바꿔, `reviewId`가 주어지면 그 특정 독후감만 편집하고 `null`이면 항상 새 독후감 작성 모드로 진입하도록 변경(더 이상 "가장 최근 것"을 추측하지 않음). 책 상세/독후감 전체보기 화면의 "작성" 버튼은 `reviewId` 없이, 각 `ReviewCard`의 "수정" 버튼은 해당 카드의 `reviewId`와 함께 `ReviewEditorScreen`으로 진입.
  - 내비게이션: `Destinations.REVIEW_EDITOR`가 `"book/{bookId}/review?reviewId={reviewId}"`로 바뀌어 선택적 쿼리 파라미터를 가짐(Navigation-Compose엔 nullable `Long` 인자 타입이 없어 `NO_REVIEW_ID = -1L`을 "새 글" sentinel로 사용). `QUOTE_LIST`("book/{bookId}/quotes")/`REVIEW_LIST`("book/{bookId}/reviews") 라우트를 새로 추가하고, `BooklogsNavHost`에서 둘 다 책 상세와 동일한 `bookDetailViewModel`을 그대로 넘겨줌.
  - `ReviewCard`에 평점(★)과 수정/삭제 버튼을 추가(기존엔 `QuoteCard`와 달리 읽기 전용이었음), `BookDetailViewModel.deleteReview()` 신설.
- **라운드 이력에 삭제 기능 추가**: 지금까지 "라운드 이력"에서 날짜/사유/시작 페이지는 고칠 수 있었지만 라운드 자체를 지울 방법은 없었음(예: 완독/중단을 실수로 눌러 만들어진 빈 라운드를 치우고 싶은 경우) — 새 `DeleteRoundUseCase`를 추가해 `RoundRow`가 펼쳐졌을 때 "취소"/"저장" 옆에 "삭제" 버튼을 노출. `EditRoundUseCase`와 같은 원칙으로 **종료된(닫힌) 라운드만** 삭제 가능하고, 현재 진행 중인 라운드는 버튼 자체를 숨김(그 라운드를 지우면 READING/PAUSED 책에 대응하는 열린 라운드가 없어져 상태 머신과 어긋나므로). 라운드가 삭제되면 그 라운드에 딸린 `ReadingLogEntity` 행들도 기존 FK(`onDelete=CASCADE`)에 의해 자동으로 함께 삭제됨 — 별도 정리 로직 불필요. 삭제 전 "이 라운드에 기록된 진행 이력도 함께 삭제돼요" 확인 다이얼로그를 거침(책/인용구/독후감 삭제와 동일한 패턴).
- **남아있던 연보라(Material 3 baseline) 색상 전부 회색으로 교체**: 위 두 차례("색상 토큰 버그 수정", "연보라 컨테이너 색상 → 회색 통일")의 패치가 그때그때 눈에 띈 요소만 고쳤던 탓에, `Theme.kt`가 여전히 `primary`/`background`/`surface`/`surfaceVariant`/`primaryContainer`/`secondaryContainer`/`tertiaryContainer`(+각 `onXContainer`)/`outline`/`error`만 오버라이드하고 있었음 — `QuoteCard`/`ReviewCard`가 쓰는 `surfaceContainerHigh`, `BookConfirmFormScreen`이 쓰는 `onSurfaceVariant`처럼 코드에서 직접 참조하는 롤은 물론, 어떤 화면도 이름으로 부르지 않지만 `ModalBottomSheet`(댓글/공백조정/빠른기록 시트)·`SnackbarHost`가 내부적으로 읽는 `surfaceContainerLow`/`inverseSurface`/`inverseOnSurface`/`inversePrimary`, `secondary`/`tertiary`, `outlineVariant`, `scrim`, 나머지 `surfaceContainer*`/`surfaceBright`/`surfaceDim`까지 전부 `lightColorScheme()`/`darkColorScheme()`의 보라색 시드 기본값으로 새고 있었음. `Color.kt`에 `SurfaceContainerLowLight`(`#F2F2F7`, iOS `systemGray6`)/`SurfaceContainerHighestLight`(`#C7C7CC`, `systemGray3`)/`SurfaceContainerHighestDark`(`#636366`, `systemGray2`)/`ScrimColor`(순검정)만 새로 추가하고 나머지는 기존 회색 팔레트(`SurfaceVariant`/`ContainerAccent`/`Muted`/`Canvas`/`OnSurface`)를 재사용해 `ColorScheme`의 모든 파라미터를 명시적으로 채움 — 앞으로 새 화면이 어떤 롤을 쓰든 더 이상 보라색이 새어나올 수 없음.
- **상태바/내비게이션바 아이콘 색이 앱 테마 설정과 어긋나는 버그 수정**: "설정 > 화면 테마" 기능은 Compose `MaterialTheme`(배경/글자색)만 `themeMode`로 제어하고, 상태바/내비게이션바 아이콘의 밝기(어두운 아이콘 vs 밝은 아이콘)는 손대지 않고 있었음 — `themes.xml`의 `Theme.Booklogs`가 `DayNight`가 아닌 항상-라이트 테마(`android:Theme.Material.Light.NoActionBar`)라 OS가 시스템 다크모드 설정만 보고 아이콘 밝기를 정했음. 그래서 **시스템 설정과 앱 내 테마 설정이 어긋나면**(예: 시스템은 다크인데 앱 설정은 라이트, 또는 그 반대) 실제 배경색과 반대의 아이콘 색이 얹혀 상태바가 안 보이는 문제가 있었음. `BooklogsTheme`에 `WindowCompat.getInsetsController(window, view)`로 `isAppearanceLightStatusBars`/`isAppearanceLightNavigationBars`를 `themeMode`로 이미 계산해둔 `darkTheme` 값에 맞춰 직접 설정하는 `SideEffect`를 추가 — 이제 시스템 설정과 무관하게 항상 실제로 보이는 배경(`darkTheme`)과 아이콘 밝기가 일치함.
