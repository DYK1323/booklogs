...
- **메타데이터 연동(카카오 메인 + Google Books 병렬 보조) — 실패 처리 상세**: 각 API 호출은 개별적으로 `ApiLookupResult<T>`(`Success(data)` / `NotFound`(정상 응답, 결과 0건) / `NetworkError`(타임아웃·오프라인·5xx))를 반환. `BookMetadataRepositoryImpl.lookup(isbn 또는 query)`가 코루틴 `async`로 카카오·Google Books를 **동시에** 호출 후(각 5초 타임아웃, 병렬이라 전체 최악 대기시간도 5초 — 순차였다면 최대 10초) 두 결과를 조합:
  - 카카오 `Success` + Google Books `Success` → 카카오의 title/author/publisher/coverImageUrl/isbn을 채택하고 Google Books의 `pageCount`→`totalPages`, `categories`→`genre`만 덧씌움(정상 케이스).
  ...
  - **제목 검색 목록 단계**는 매 검색마다 두 API를 병렬 호출하면 호출 수가 과해지므로 **카카오 단독 검색**이 우선이고, 카카오 결과가 0건(`NotFound`)일 때만 동일 검색어로 Google Books 검색을 재시도해 목록을 채움. 사용자가 목록에서 항목을 하나 고른 시점에야 그 책에 대해 Google Books를 페이지수/장르용으로 추가 조회(위 4가지 분기와 동일하게 처리).
